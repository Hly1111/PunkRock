...
int32 UAkAudioEvent::GetSourcePlayPosition(int32 PlayingID)
{
	...
	{
		GEngine->AddOnScreenDebugMessage(-1, 2.f, FColor::Red, TEXT("Failed to get source play position for AkAudioEvent without an Audio Device!"));
		return -1;
	}
	...
	{
		GEngine->AddOnScreenDebugMessage(-1, 2.f, FColor::Red, TEXT("Failed to get source play position for AkAudioEvent with the Sound Engine not initialized!"));
		return -1;
	}

	auto* SoundEngine = IWwiseSoundEngineAPI::Get();

	// Get the current play position directly from SoundEngine
	AkTimeMs CurrentPosition = 0;
	auto Result = SoundEngine->GetSourcePlayPosition(PlayingID, &CurrentPosition);
	if (Result == AK_Success)
	{
		return static_cast<int32>(CurrentPosition);
	}
	else
	{
